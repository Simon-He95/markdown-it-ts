...
下面对比 markdown-it-ts 在同规模下最快 one-shot 场景与 `@ox-content/napi` 的 native parse-only 基线。

注意：`@ox-content/napi` 的 parse-only API 返回 AST JSON 字符串；下面的 parse-only 数据不包含额外 `JSON.parse` 成 JavaScript 对象的成本。

<!-- perf-auto:ox-one:start -->
- 5,000 chars: 0.0392ms vs 0.0391ms → ~1 倍更慢，约多 0% 耗时
- 20,000 chars: 0.1274ms vs 0.1500ms → ~1.2× 更快，约少 15% 耗时
- 100,000 chars: 0.7522ms vs 0.9136ms → ~1.2× 更快，约少 18% 耗时
<!-- perf-auto:ox-one:end -->

如果把 `@ox-content/napi` 返回的 AST JSON 字符串立即 `JSON.parse` 成 JavaScript 对象：

<!-- perf-auto:ox-json-one:start -->
- 5,000 chars: 0.0392ms vs 0.1735ms → ~4.4× 更快，约少 77% 耗时
- 20,000 chars: 0.1274ms vs 0.6957ms → ~5.5× 更快，约少 82% 耗时
- 100,000 chars: 0.7522ms vs 3.6922ms → ~4.9× 更快，约少 80% 耗时
...
- 5,000 chars: 0.0248ms vs 0.0485ms → ~2× 更快，约少 49% 耗时
- 20,000 chars: 0.0856ms vs 0.1639ms → ~1.9× 更快，约少 48% 耗时
- 100,000 chars: 0.4223ms vs 0.9715ms → ~2.3× 更快，约少 57% 耗时
<!-- perf-auto:stock-ast-json:end -->

从 native 基线里能学到的优化方向：

- `@ox-content/napi` 的 parse-only 很快，关键原因是返回紧凑的 AST JSON 字符串，而不是在 JavaScript 里物化大量 `Token[]` 对象图。
- 一旦把这段 JSON 字符串继续 `JSON.parse` 成 JavaScript 对象，本 benchmark 里 markdown-it-ts 反而更快。
- 实验性的 `parseStockFastAstJson` 也验证了同一个方向：如果调用方能消费紧凑 AST/string 边界，就应该避免 JS Token 分配。
- 默认 `md.parse()` 仍然必须返回 markdown-it 兼容、可变的 token，因此剩余差距主要来自 Token 对象、children 数组、source-map 数组和 GC 成本，而不是简单的行扫描。
...
- 5,000 chars: 0.0392ms vs 3.8400ms → 98× faster
- 20,000 chars: 0.1274ms vs 20.37ms → 159.9× faster
- 100,000 chars: 0.7522ms vs 149.88ms → 199.2× faster
<!-- perf-auto:remark-one:end -->

增量工作负载（append workload）：

<!-- perf-auto:remark-append:start -->
- 5,000 chars: 0.1074ms vs 12.71ms → 118.3× faster
- 20,000 chars: 0.4377ms vs 62.28ms → 142.3× faster
- 100,000 chars: 2.4980ms vs 474.24ms → 189.8× faster
...
- 5,000 chars: 0.0392ms vs 3.7790ms → 96.5× faster
- 20,000 chars: 0.1274ms vs 16.72ms → 131.3× faster
- 100,000 chars: 0.7522ms vs 93.21ms → 123.9× faster
...
- 5,000 chars: 0.1074ms vs 12.19ms → 113.5× faster
- 20,000 chars: 0.4377ms vs 48.94ms → 111.8× faster
- 100,000 chars: 2.4980ms vs 314.09ms → 125.7× faster
...
下面对比默认 `markdown-it-ts.render` 与 `@ox-content/napi` 的 native parse + render 基线。

<!-- perf-auto:render-ox:start -->
- 5,000 chars: 0.0208ms vs 0.0363ms → ~1.7× 更快，约少 43% 耗时
- 20,000 chars: 0.0723ms vs 0.1535ms → ~2.1× 更快，约少 53% 耗时
- 100,000 chars: 0.3438ms vs 0.8941ms → ~2.6× 更快，约少 62% 耗时
...
|---:|---:|---:|:--|---:|---:|:--|
| 5,000 | 0.0392ms | 0.0391ms | ~1 倍更慢，约多 0% 耗时 | 0.0208ms | 0.0363ms | ~1.7× 更快，约少 43% 耗时 |
| 20,000 | 0.1274ms | 0.1500ms | ~1.2× 更快，约少 15% 耗时 | 0.0723ms | 0.1535ms | ~2.1× 更快，约少 53% 耗时 |
| 100,000 | 0.7522ms | 0.9136ms | ~1.2× 更快，约少 18% 耗时 | 0.3438ms | 0.8941ms | ~2.6× 更快，约少 62% 耗时 |
<!-- perf-auto:ox-summary:end -->

### 对比 markdown-it render API

<!-- perf-auto:render-md:start -->
- 5,000 chars: 0.0208ms vs 0.2313ms → ~11.1× faster
- 20,000 chars: 0.0723ms vs 0.9083ms → ~12.6× faster
- 100,000 chars: 0.3438ms vs 7.3312ms → ~21.3× faster
- 500,000 chars: 2.5149ms vs 32.50ms → ~12.9× faster
- 1,000,000 chars: 5.0665ms vs 65.75ms → ~13× faster
<!-- perf-auto:render-md:end -->

### 对比 remark + rehype render API

<!-- perf-auto:render-remark:start -->
- 5,000 chars: 0.0208ms vs 4.6784ms → ~225.4× faster
- 20,000 chars: 0.0723ms vs 22.59ms → ~312.4× faster
- 100,000 chars: 0.3438ms vs 262.57ms → ~763.7× faster
<!-- perf-auto:render-remark:end -->

### 对比 micromark（CommonMark 参考实现）

<!-- perf-auto:render-micromark:start -->
- 5,000 chars: 0.0208ms vs 3.8839ms → ~187.1× faster
- 20,000 chars: 0.0723ms vs 17.92ms → ~247.9× faster
- 100,000 chars: 0.3438ms vs 137.27ms → ~399.3× faster
...
|---:|---:|---:|
| 5,000 | 0.0392ms | 0.2542ms |
| 20,000 | 0.1274ms | 1.0026ms |
| 50,000 | 0.3065ms | 2.5182ms |
| 100,000 | 0.7522ms | 5.3012ms |
| 200,000 | 1.7520ms | 11.85ms |
...
- 5,000 chars: 0.0208ms vs 0.2994ms → ~14.4× faster
- 20,000 chars: 0.0723ms vs 1.1767ms → ~16.3× faster
- 50,000 chars: 0.1750ms vs 2.9559ms → ~16.9× faster
- 100,000 chars: 0.3438ms vs 10.18ms → ~29.6× faster
- 200,000 chars: 0.6955ms vs 13.96ms → ~20.1× faster
...
为了更直观地查看主要实现（markdown-it-ts、@ox-content/napi、markdown-it、markdown-exit、remark）在不同规模下的 parse / render 名次，下面直接基于最新 `docs/perf-latest.json` 的快照生成。
