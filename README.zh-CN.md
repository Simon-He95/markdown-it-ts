...
|:--|---:|---:|---:|:--|---:|---:|:--|:--|
| synthetic stock-subset (~100k) | 100,126 | 0.5848ms | 0.7848ms | stock-fast | 0.2728ms | 0.7344ms | stock-fast | 否 |
| synthetic feature-mixed (~100k) | 100,450 | 3.7729ms | 0.9029ms | general | 4.5379ms | 0.8322ms | token-renderer | 否 |
| docs/architecture.md | 6,564 | 0.0943ms | 0.0212ms | general | 0.1020ms | 0.0152ms | token-renderer | 否 |
| docs/development.md | 4,756 | 0.1012ms | 0.0209ms | general | 0.1151ms | 0.0174ms | token-renderer | 否 |
| docs/security.md | 1,375 | 0.0270ms | 0.0064ms | general | 0.0299ms | 0.0058ms | token-renderer | 否 |
...
- 5,000 chars: 0.0287ms vs 0.0305ms → ~1.1× 更快，约少 6% 耗时
- 20,000 chars: 0.1011ms vs 0.1221ms → ~1.2× 更快，约少 17% 耗时
- 100,000 chars: 0.5720ms vs 0.7109ms → ~1.2× 更快，约少 20% 耗时
<!-- perf-auto:ox-one:end -->

如果把 `@ox-content/napi` 返回的 AST JSON 字符串立即 `JSON.parse` 成 JavaScript 对象：

<!-- perf-auto:ox-json-one:start -->
- 5,000 chars: 0.0287ms vs 0.1414ms → ~4.9× 更快，约少 80% 耗时
- 20,000 chars: 0.1011ms vs 0.5610ms → ~5.5× 更快，约少 82% 耗时
- 100,000 chars: 0.5720ms vs 2.8824ms → ~5× 更快，约少 80% 耗时
...
- 5,000 chars: 0.0193ms vs 0.0311ms → ~1.6× 更快，约少 38% 耗时
- 20,000 chars: 0.0685ms vs 0.1303ms → ~1.9× 更快，约少 47% 耗时
- 100,000 chars: 0.3296ms vs 0.8047ms → ~2.4× 更快，约少 59% 耗时
...
- 5,000 chars: 0.0287ms vs 3.0759ms → 107.3× faster
- 20,000 chars: 0.1011ms vs 17.10ms → 169× faster
- 100,000 chars: 0.5720ms vs 123.25ms → 215.5× faster
<!-- perf-auto:remark-one:end -->

增量工作负载（append workload）：

<!-- perf-auto:remark-append:start -->
- 5,000 chars: 0.0889ms vs 10.44ms → 117.4× faster
- 20,000 chars: 0.3305ms vs 51.30ms → 155.2× faster
- 100,000 chars: 2.0591ms vs 403.13ms → 195.8× faster
...
- 5,000 chars: 0.0287ms vs 3.2612ms → 113.8× faster
- 20,000 chars: 0.1011ms vs 14.11ms → 139.5× faster
- 100,000 chars: 0.5720ms vs 81.69ms → 142.8× faster
...
- 5,000 chars: 0.0889ms vs 10.10ms → 113.7× faster
- 20,000 chars: 0.3305ms vs 45.94ms → 139× faster
- 100,000 chars: 2.0591ms vs 279.23ms → 135.6× faster
...
- 5,000 chars: 0.0163ms vs 0.0284ms → ~1.7× 更快，约少 43% 耗时
- 20,000 chars: 0.0562ms vs 0.1266ms → ~2.3× 更快，约少 56% 耗时
- 100,000 chars: 0.2689ms vs 0.6686ms → ~2.5× 更快，约少 60% 耗时
...
|---:|---:|---:|:--|---:|---:|:--|
| 5,000 | 0.0287ms | 0.0305ms | ~1.1× 更快，约少 6% 耗时 | 0.0163ms | 0.0284ms | ~1.7× 更快，约少 43% 耗时 |
| 20,000 | 0.1011ms | 0.1221ms | ~1.2× 更快，约少 17% 耗时 | 0.0562ms | 0.1266ms | ~2.3× 更快，约少 56% 耗时 |
| 100,000 | 0.5720ms | 0.7109ms | ~1.2× 更快，约少 20% 耗时 | 0.2689ms | 0.6686ms | ~2.5× 更快，约少 60% 耗时 |
<!-- perf-auto:ox-summary:end -->

### 对比 markdown-it render API

<!-- perf-auto:render-md:start -->
- 5,000 chars: 0.0163ms vs 0.1835ms → ~11.3× faster
- 20,000 chars: 0.0562ms vs 0.7292ms → ~13× faster
- 100,000 chars: 0.2689ms vs 3.9406ms → ~14.7× faster
- 500,000 chars: 2.0176ms vs 29.99ms → ~14.9× faster
- 1,000,000 chars: 4.1138ms vs 54.41ms → ~13.2× faster
<!-- perf-auto:render-md:end -->

### 对比 remark + rehype render API

<!-- perf-auto:render-remark:start -->
- 5,000 chars: 0.0163ms vs 3.7103ms → ~227.8× faster
- 20,000 chars: 0.0562ms vs 18.95ms → ~337.3× faster
- 100,000 chars: 0.2689ms vs 140.09ms → ~520.9× faster
<!-- perf-auto:render-remark:end -->

### 对比 micromark（CommonMark 参考实现）

<!-- perf-auto:render-micromark:start -->
- 5,000 chars: 0.0163ms vs 3.1072ms → ~190.8× faster
- 20,000 chars: 0.0562ms vs 14.82ms → ~263.8× faster
- 100,000 chars: 0.2689ms vs 94.83ms → ~352.6× faster
...
|---:|---:|---:|
| 5,000 | 0.0287ms | 0.2008ms |
| 20,000 | 0.1011ms | 0.8356ms |
| 50,000 | 0.2474ms | 2.0827ms |
| 100,000 | 0.5720ms | 4.3115ms |
| 200,000 | 1.5858ms | 9.3080ms |
...
- 5,000 chars: 0.0163ms vs 0.2322ms → ~14.3× faster
- 20,000 chars: 0.0562ms vs 0.9268ms → ~16.5× faster
- 50,000 chars: 0.1374ms vs 2.3800ms → ~17.3× faster
- 100,000 chars: 0.2689ms vs 4.9653ms → ~18.5× faster
- 200,000 chars: 0.5401ms vs 11.10ms → ~20.5× faster
