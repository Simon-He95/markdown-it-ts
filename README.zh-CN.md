...
|:--|---:|---:|---:|:--|---:|---:|:--|:--|
| synthetic stock-subset (~100k) | 100,126 | 0.6366ms | 0.8393ms | stock-fast | 0.3525ms | 0.7492ms | stock-fast | 否 |
| synthetic feature-mixed (~100k) | 100,450 | 4.2638ms | 1.1269ms | general | 5.7439ms | 1.0683ms | token-renderer | 否 |
| docs/architecture.md | 6,564 | 0.0888ms | 0.0314ms | general | 0.1027ms | 0.0250ms | token-renderer | 否 |
| docs/development.md | 4,756 | 0.0984ms | 0.0294ms | general | 0.1149ms | 0.0256ms | token-renderer | 否 |
| docs/security.md | 1,375 | 0.0272ms | 0.0089ms | general | 0.0331ms | 0.0082ms | token-renderer | 否 |
...
- 5,000 chars: 0.0400ms vs 0.1860ms → ~4.6× faster, ~78% less time
- 20,000 chars: 0.1175ms vs 0.7358ms → ~6.3× faster, ~84% less time
- 100,000 chars: 0.6514ms vs 3.7884ms → ~5.8× faster, ~83% less time
- 500,000 chars: 6.8542ms vs 27.50ms → ~4× faster, ~75% less time
- 1,000,000 chars: 14.33ms vs 121.67ms → ~8.5× faster, ~88% less time
...
- 5,000 chars: 0.0400ms vs 0.0431ms → ~1.1× 更快，约少 7% 耗时
- 20,000 chars: 0.1175ms vs 0.1626ms → ~1.4× 更快，约少 28% 耗时
- 100,000 chars: 0.6514ms vs 0.8422ms → ~1.3× 更快，约少 23% 耗时
<!-- perf-auto:ox-one:end -->

如果把 `@ox-content/napi` 返回的 AST JSON 字符串立即 `JSON.parse` 成 JavaScript 对象：

<!-- perf-auto:ox-json-one:start -->
- 5,000 chars: 0.0400ms vs 0.1840ms → ~4.6× 更快，约少 78% 耗时
- 20,000 chars: 0.1175ms vs 0.7184ms → ~6.1× 更快，约少 84% 耗时
- 100,000 chars: 0.6514ms vs 3.5881ms → ~5.5× 更快，约少 82% 耗时
...
- 5,000 chars: 0.0262ms vs 0.0482ms → ~1.8× 更快，约少 46% 耗时
- 20,000 chars: 0.1007ms vs 0.2012ms → ~2× 更快，约少 50% 耗时
- 100,000 chars: 0.5264ms vs 1.1671ms → ~2.2× 更快，约少 55% 耗时
...
- 5,000 chars: 0.0400ms vs 3.7666ms → 94.1× faster
- 20,000 chars: 0.1175ms vs 19.97ms → 170× faster
- 100,000 chars: 0.6514ms vs 143.79ms → 220.7× faster
<!-- perf-auto:remark-one:end -->

增量工作负载（append workload）：

<!-- perf-auto:remark-append:start -->
- 5,000 chars: 0.1069ms vs 12.31ms → 115.1× faster
- 20,000 chars: 0.3998ms vs 62.36ms → 156× faster
- 100,000 chars: 2.3352ms vs 459.35ms → 196.7× faster
...
- 5,000 chars: 0.0400ms vs 3.0594ms → 76.4× faster
- 20,000 chars: 0.1175ms vs 16.35ms → 139.2× faster
- 100,000 chars: 0.6514ms vs 91.53ms → 140.5× faster
...
- 5,000 chars: 0.1069ms vs 10.40ms → 97.3× faster
- 20,000 chars: 0.3998ms vs 48.35ms → 120.9× faster
- 100,000 chars: 2.3352ms vs 310.25ms → 132.9× faster
...
- 5,000 chars: 0.0215ms vs 0.0389ms → ~1.8× 更快，约少 45% 耗时
- 20,000 chars: 0.0727ms vs 0.1525ms → ~2.1× 更快，约少 52% 耗时
- 100,000 chars: 0.3541ms vs 0.7653ms → ~2.2× 更快，约少 54% 耗时
...
|---:|---:|---:|:--|---:|---:|:--|
| 5,000 | 0.0400ms | 0.0431ms | ~1.1× 更快，约少 7% 耗时 | 0.0215ms | 0.0389ms | ~1.8× 更快，约少 45% 耗时 |
| 20,000 | 0.1175ms | 0.1626ms | ~1.4× 更快，约少 28% 耗时 | 0.0727ms | 0.1525ms | ~2.1× 更快，约少 52% 耗时 |
| 100,000 | 0.6514ms | 0.8422ms | ~1.3× 更快，约少 23% 耗时 | 0.3541ms | 0.7653ms | ~2.2× 更快，约少 54% 耗时 |
<!-- perf-auto:ox-summary:end -->

### 对比 markdown-it render API

<!-- perf-auto:render-md:start -->
- 5,000 chars: 0.0215ms vs 0.2331ms → ~10.8× faster
- 20,000 chars: 0.0727ms vs 0.9347ms → ~12.9× faster
- 100,000 chars: 0.3541ms vs 4.8068ms → ~13.6× faster
- 500,000 chars: 2.7205ms vs 34.48ms → ~12.7× faster
- 1,000,000 chars: 5.3894ms vs 97.52ms → ~18.1× faster
<!-- perf-auto:render-md:end -->

### 对比 remark + rehype render API

<!-- perf-auto:render-remark:start -->
- 5,000 chars: 0.0215ms vs 4.6822ms → ~217.5× faster
- 20,000 chars: 0.0727ms vs 23.15ms → ~318.6× faster
- 100,000 chars: 0.3541ms vs 164.29ms → ~464× faster
<!-- perf-auto:render-remark:end -->

### 对比 micromark（CommonMark 参考实现）

<!-- perf-auto:render-micromark:start -->
- 5,000 chars: 0.0215ms vs 3.8381ms → ~178.3× faster
- 20,000 chars: 0.0727ms vs 19.12ms → ~263.1× faster
- 100,000 chars: 0.3541ms vs 114.41ms → ~323.1× faster
...
|---:|---:|---:|
| 5,000 | 0.0400ms | 0.2583ms |
| 20,000 | 0.1175ms | 1.0007ms |
| 50,000 | 0.2929ms | 2.5201ms |
| 100,000 | 0.6514ms | 5.3355ms |
| 200,000 | 1.7521ms | 11.82ms |
...
- 5,000 chars: 0.0215ms vs 0.3147ms → ~14.6× faster
- 20,000 chars: 0.0727ms vs 1.2865ms → ~17.7× faster
- 50,000 chars: 0.1750ms vs 2.9785ms → ~17× faster
- 100,000 chars: 0.3541ms vs 6.1652ms → ~17.4× faster
- 200,000 chars: 0.7094ms vs 13.26ms → ~18.7× faster
...
|---:|---:|---|---:|
| 5,000 | 1 | markdown-it-ts | 0.0400ms |
| 5,000 | 2 | @ox-content/napi | 0.0431ms |
| 5,000 | 3 | markdown-it | 0.1860ms |
| 5,000 | 4 | markdown-exit | 0.2583ms |
| 5,000 | 5 | remark | 3.7666ms |
| 20,000 | 1 | markdown-it-ts | 0.1175ms |
| 20,000 | 2 | @ox-content/napi | 0.1626ms |
| 20,000 | 3 | markdown-it | 0.7358ms |
| 20,000 | 4 | markdown-exit | 1.0007ms |
| 20,000 | 5 | remark | 19.97ms |
| 50,000 | 1 | markdown-it-ts | 0.2929ms |
| 50,000 | 2 | @ox-content/napi | 0.4321ms |
| 50,000 | 3 | markdown-it | 1.8372ms |
| 50,000 | 4 | markdown-exit | 2.5201ms |
| 50,000 | 5 | remark | 65.33ms |
| 100,000 | 1 | markdown-it-ts | 0.6514ms |
| 100,000 | 2 | @ox-content/napi | 0.8422ms |
| 100,000 | 3 | markdown-it | 3.7884ms |
| 100,000 | 4 | markdown-exit | 5.3355ms |
| 100,000 | 5 | remark | 143.79ms |
| 200,000 | 1 | @ox-content/napi | 1.6949ms |
| 200,000 | 2 | markdown-it-ts | 1.7521ms |
| 200,000 | 3 | markdown-it | 8.7912ms |
| 200,000 | 4 | markdown-exit | 11.82ms |
| 200,000 | 5 | remark | 464.54ms |

**Render 排名（解析 + HTML 输出耗时，单位：ms）**

| Size | Rank | Library | renderMs |
|---:|---:|---|---:|
| 5,000 | 1 | markdown-it-ts | 0.0215ms |
| 5,000 | 2 | @ox-content/napi | 0.0389ms |
| 5,000 | 3 | markdown-it | 0.2331ms |
| 5,000 | 4 | markdown-exit | 0.3147ms |
| 5,000 | 5 | remark + rehype | 4.6822ms |
| 20,000 | 1 | markdown-it-ts | 0.0727ms |
| 20,000 | 2 | @ox-content/napi | 0.1525ms |
| 20,000 | 3 | markdown-it | 0.9347ms |
| 20,000 | 4 | markdown-exit | 1.2865ms |
| 20,000 | 5 | remark + rehype | 23.15ms |
| 50,000 | 1 | markdown-it-ts | 0.1750ms |
| 50,000 | 2 | @ox-content/napi | 0.3735ms |
| 50,000 | 3 | markdown-it | 2.3149ms |
| 50,000 | 4 | markdown-exit | 2.9785ms |
| 50,000 | 5 | remark + rehype | 72.20ms |
| 100,000 | 1 | markdown-it-ts | 0.3541ms |
| 100,000 | 2 | @ox-content/napi | 0.7653ms |
| 100,000 | 3 | markdown-it | 4.8068ms |
| 100,000 | 4 | markdown-exit | 6.1652ms |
| 100,000 | 5 | remark + rehype | 164.29ms |
| 200,000 | 1 | markdown-it-ts | 0.7094ms |
| 200,000 | 2 | @ox-content/napi | 1.5276ms |
| 200,000 | 3 | markdown-it | 11.39ms |
| 200,000 | 4 | markdown-exit | 13.26ms |
| 200,000 | 5 | remark + rehype | 413.92ms |
